...
```
It's always safe (in the sense that no information will be lost) to turn a `u32` into an `i64`, but not the other way round. If the `i64` is negative, or larger than the maximum `u32`, there is no correct answer for this operation. However, Rust has [rules](https://doc.rust-lang.org/reference/expressions/operator-expr.html#semantics) about what to do here. Essentially, casting down to a smaller type 'truncates' the integer, keeping only the last 32 bits. Negative values get a little bit complicated, we'll brush over it for now.
