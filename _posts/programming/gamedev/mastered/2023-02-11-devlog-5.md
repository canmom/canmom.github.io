...
There is a small but noticeable increase in the frame time when we move the mouse and rotate the camera. Interestingly, this persists even when I turn off the systems relating to this.

However, the editor loop is going to confuse things. To profile properly we need to make a build.

There was a small issue with Latios not building correctly, but Dreaming filled me in on how to fix it. If you get the bug

> Library/PackageCache/com.latios.latiosframework@0.6.4/EntitiesExposed/Hybrid/BakerExposed.cs(52,34): error CS0234: The type or namespace name 'TypeCache' does not exist in the namespace 'UnityEditor' (are you missing an assembly reference?)

The solution is to add a `#if UNITY_EDITOR` and `endif` directive around the for loops in the offending file. Should be fixed in the next version.

That done, we can do a bit of profiling.
