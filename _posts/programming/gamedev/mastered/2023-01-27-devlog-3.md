...
excerpt: Unity DOTS support for skinned meshes is not quite there. How do we solve this?
layout: article
categories:
 - programming
 - gamedev
...
{% include figure.html alt="A Unity window with an unholy mess of polygons." capt="19 Jan 2023. An attempt to place the doll in the intended animation position using Rigify's IK system. There are deformation issues at the hips, and proportion issues in general." src="embed/gamedev/thrustdoll/dev/rig-testing-3.png" %}
...
I spoke to Dreaming about this issue, and he advised me to call `CompleteDependency`---which turns out to be a method on the `EntityQuery`. However, this didn't solve the issue.
...
A more sophisticated solution would be to insert the relevant references into the `Dependency` of the second system. We could stash the `jobHandle` in some kind of singleton entity, or we could tell Unity that this system is going to be doing stuff with transforms, so the job will wait for any other jobs touching transforms to finish. However, I'm going to hold off on doing that until Latios Transforms comes out, since I'm going to have to rewrite all my transform-related code again anyway.
