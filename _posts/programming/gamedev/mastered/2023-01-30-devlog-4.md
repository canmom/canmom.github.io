---
title: "THRUST//DOLL devlog 3: the movement system"
...
public partial class CameraControllerSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        quaternion rotation = quaternion.EulerXYZ(0f,intendedRotation.y, intendedRotation.x);
...
protected override void OnUpdate()
{
    ...
    quaternion rotation = quaternion.EulerXYZ(controller.ValueRO.Pitch,controller.ValueRO.Yaw,0f);
...
{% include video.html src="embed/img/thrustdoll/dev/video/rotation-around-character.mp4" %}
...
Damn, feels almost like we're writing Rust now! So now we can create a job to countdown any temporary status effects we wish to apply in the future.
...
partial struct StatusTickSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        new StatusCountdownJob<Thrust> { DeltaTime = deltaTime }.Schedule();
        new StatusCountdownJob<ThrustCooldown> { DeltaTime = deltaTime }.Schedule();
    }
}
...
```
Try compiling this and, surprise surprise! You can't use SystemAPI with generics.
