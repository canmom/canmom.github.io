...
To my mind, the next step comes from placing LLMs in a world-simulation (distinct from the character-language simulation that the LLMs themselves represent): somewhere where they can receive something like sensory input (descriptions of their surroundings), and act on that environment, producing consequences, and update their internal representation based on what they 'see'. This is accomplished through the use of an agent framework (see below). Of course, we are limited by the fact that the models can only 'learn' at inference time in very limited ways, like a human with severe amnesia who forgets anything they don't write down immediately---though perhaps the upcoming generation of 'liquid' models might change that.
