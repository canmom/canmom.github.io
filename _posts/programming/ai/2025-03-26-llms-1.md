...
excerpt: I get up to speed on some of the things you can do to raise a pile of floating point numbers
layout: article
categories:
 - programming
 - ai
...
This comes from placing LLMs in a simulation: somewhere where they can receive something like sensory input (descriptions of their surroundings), and act on that environment, producing consequences. This is accomplished through the use of an agent framework, for example [langchain](https://python.langchain.com/docs/introduction/), in which a model is given the ability to turn over its actions and generate instructions to use certain tools, run code, or apply other tricks to help reach its answer.
...
In the next post, we'll go over the various models I've played with so far, and their quirks!